...
async function verifyToken(token, secret) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (data?.expired) {
                await generateCustomError('token expired !', 'auth_token_error', 401, 'authTokenExpired');
            }
            resolve(data);
        } catch (error) {
            reject(error);
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (!('authorization' in req.headers))
                await generateCustomError('malformed authorization !', 'auth_error', 400, 'invalidHeader');

            let { authorization: token } = req.headers;

            //? checking if header value contains Bearer
            if (!token.startsWith('Bearer')) await generateCustomError('malformed authorization !', 'auth_error', 401, 'noBearer');
...
async function compareHash(userPassword, hash) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            await generateCustomError('incorrect username or password', 'auth_error', 401, 'invalidCredentials');
        } catch (error) {
            reject(error);
        }
    });
